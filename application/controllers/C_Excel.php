...
class C_Excel extends CI_Controller {
	...
	public function Excel_Pembayaran(){
		...
		$excel->getProperties()->setCreator('PEMBAYARAN SPP SMEA')
							   ->setLastModifiedBy('PEMBAUARAN SPP SMEA 2020')
		...
		$pembayaran = $this->MPembayaran->join_table($data_cari);
		...
		foreach($pembayaran as $data){ 
			...
			$excel->setActiveSheetIndex(0)->setCellValue('F'.$numrow, 'Tahun Ke '.$data['tahun_bayar']);
			$excel->setActiveSheetIndex(0)->setCellValue('G'.$numrow, $data['bulan_bayar'].' Bulan');
